...
class UserNameImportValidator(ImportValidators):
    ...
    def validate(self) -> ValidationResponse:
        ...
        try:
            ...
            logging.info("HTTP Error status code: %s", e.response.status_code)
            if e.response.status_code == 404:
                try:
                    error_details = e.response.json()
                    logging.error("Username does not exist: %s", error_details)
                except (ValueError, KeyError):
                    logging.error("Username does not exist (status code: %s)", e.response.status_code)
                return ValidationResponse(
                    validation_name=self.validation_name,
                ...
                try:
                    error_details = e.response.json()
                    logging.error("Unauthorized for url: %s", error_details)
                except (ValueError, KeyError):
                    logging.error("Unauthorized (status code: %s)", e.response.status_code)
                return ValidationResponse(
                    validation_name=self.validation_name,
